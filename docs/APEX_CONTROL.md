...
same way. Guarded by `CONFIG_APEX_G4B_SHELL`, which is on in both the debug and
the release build (see §9 for the release path).
...
| `apex link` | scanner health: **boot handshake** (one-shot 59/59 frames, proves the link came up, frozen after handover), plus **live** counters from the permanent keyboard loop: `key events` (accepted key state changes, climbs as you type) and `idle keepalives` (0xA0 polls between events). *The old "ingest calls/ok" fields were boot-probe snapshots that froze at 0 after handover and falsely looked like keys weren't ingesting, so watch the live counters instead.* |
...
| `apex mode [bt\|usb\|dongle\|auto]` | **force the transport regardless of the physical switch**; persisted (NVS + NOINIT). USB↔BT instant; crossing dongle mode reboots to reassign radio ownership between ZMK's BLE stack and the 2.4 GHz radio, and the override survives the reset (NOINIT RAM + NVS). |
...
Rules baked in:
- **The USB rail (P0.25) is only ever *pulsed* (150 ms low to high) and always
...
non-static `zmk_studio_Response apex_<verb>(const zmk_studio_Request*)` handler, and a
...
`CONFIG_THREAD_ANALYZER`, `CONFIG_INIT_STACKS`); the shell, the tool shells, the
Studio RPC, and deferred logging all stay in.

One image carries BLE and the custom 2.4 GHz receiver link plus wireless (OTA)
